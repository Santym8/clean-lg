                    ...
                    <tr>
                        ...
                        <td>
                            @if (Gate::allows('action-allowed-to-user', ['SERVICE_ORDERS/EDIT']))
                                <a href="{{ route('service_orders.edit', ['service_order' => $service_order->id]) }}"
                                    class="btn btn-primary">Editar</a>
                            @endif
                            @if (Gate::allows('action-allowed-to-user', ['SERVICE_ORDERS/DESTROY']))
                                <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#modal{{ $service_order->id }}">Eliminar</button>

                                <div class="modal fade" id="modal{{ $service_order->id }}" tabindex="-1"
                                    aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Eliminar Orden de servicio
                                                </h5>
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No,
                                                    cancelar</button>
                                                <form
                                                    action="{{ route('service_orders.destroy', ['service_order' => $service_order->id]) }}"
                                                    method="POST">
                                                    @method('DELETE')
                                                    @csrf
